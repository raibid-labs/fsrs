...
use fsrs_frontend::ast::{BinOp, Expr, Import, Literal, ModuleDef, ModuleItem, Program};
...
fn test_compile_qualified_name() {
    ...
    assert!(chunk.constants.contains(&fsrs_vm::value::Value::Int(100)));
}
...
fn test_compile_imported_binding() {
    ...
    let chunk = Compiler::compile_program(&program).unwrap();
    assert!(!chunk.instructions.is_empty());
    assert!(chunk.constants.contains(&fsrs_vm::value::Value::Int(42)));
}
...
fn test_compile_module_with_constant() {
    ...
    assert!(chunk.constants.contains(&fsrs_vm::value::Value::Int(3)));
}
...
fn test_compile_program_using_imported_constant() {
    ...
    let chunk = Compiler::compile_program(&program).unwrap();
    assert!(!chunk.instructions.is_empty());
    assert!(chunk.constants.contains(&fsrs_vm::value::Value::Int(99)));
}
...
fn test_compile_error_undefined_module() {
    ...
    if let Err(e) = result {
        assert!(matches!(
            e,
            fsrs_frontend::compiler::CompileError::UndefinedVariable(_)
        ));
    }
}
...
fn test_compile_module_with_expression() {
    ...
    assert!(!chunk.instructions.is_empty());
    assert!(chunk.constants.contains(&fsrs_vm::value::Value::Int(3)));
    assert!(chunk.constants.contains(&fsrs_vm::value::Value::Int(4)));
}
...
fn test_compile_program_with_expression_using_module() {
    ...
    let chunk = Compiler::compile_program(&program).unwrap();
    assert!(!chunk.instructions.is_empty());
    assert!(chunk.constants.contains(&fsrs_vm::value::Value::Int(10)));
    assert!(chunk.constants.contains(&fsrs_vm::value::Value::Int(5)));
}
...
fn test_compile_imported_expression() {
    ...
    let chunk = Compiler::compile_program(&program).unwrap();
    assert!(!chunk.instructions.is_empty());
    assert!(chunk.constants.contains(&fsrs_vm::value::Value::Int(21)));
}
...
fn test_compile_module_with_bool() {
    ...
    let chunk = Compiler::compile_program(&program).unwrap();
    assert!(!chunk.instructions.is_empty());
    assert!(chunk.constants.contains(&fsrs_vm::value::Value::Bool(true)));
}
...
fn test_compile_module_with_string() {
    ...
    assert!(chunk
        .constants
        .contains(&fsrs_vm::value::Value::Str("hello".to_string())));
}
