# FSRS Host Interop API Architecture

## Overview

This document specifies the Rust host interop API for FSRS (F#-to-Rust Script Engine). The design prioritizes:
...
```rust
use fsrs::{Engine, Value, ScriptError, Result};
...
// Load scripts
engine.load_script("config.fsrs")?;
...
// Cleanup
engine.unload_script("config.fsrs")?;
```
...
```rust
// Module system
engine.load_module("plugins/theme.fsrs", "theme")?;
...
// Watch for changes
engine.watch_script("config.fsrs")?;
```
...
### 2.3 FSRS to Rust Conversion

```rust
// Trait for extracting Rust types from FSRS values
...
engine.load_script_as("config.fsrs", "app_config")?;
...
let compiled = engine.compile_script("config.fsrs")?;
std::fs::write("config.fsrs.bin", &compiled)?;

// Later, load precompiled
let compiled = std::fs::read("config.fsrs.bin")?;
...
engine.load_module_graph(&[
    "core/utils.fsrs",
    "plugins/theme.fsrs",
    "config.fsrs", // Depends on previous modules
])?;
```
...
```rust
use fsrs::hotreload::{Watcher, ReloadStrategy};
...
engine.reload_script("config.fsrs")?;
engine.reload_all()?;
...
FSRS uses a **hybrid reference counting + cycle detection** approach:
...
// Output:
//   0: compute_result at config.fsrs:42:5
//   1: process_data at config.fsrs:28:12
//   2: main at config.fsrs:10:1
```
...
impl AppConfig {
    pub fn load() -> Result<Self> {
        ...
        // Load config
        engine.load_script("app_config.fsrs")?;
